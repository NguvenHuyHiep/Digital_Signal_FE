...
export class IndexdbStorageService extends BaseStorage {
  ...
  protected get<T>(storeName: string, key?: string): Observable<T | undefined> {
    return key
      ? this.dbService.getByID<T>(storeName, key as string)
      : this.dbService
          .getAll<T>(storeName)
          .pipe(map((users) => (users ? users[0] : undefined)));
  }

  protected set<T>(storeName: string, value: T, key?: string): Observable<any> {
    if (value && value != null) {
      return key
        ? this.dbService.update(storeName, value)
        : this.dbService
            .clear(storeName)
            .pipe(switchMap(() => this.dbService.add(storeName, value)));
    }
    return key
      ? this.dbService.deleteByKey(storeName, key)
